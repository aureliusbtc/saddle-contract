...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  let nerveUSDPool = await getOrNull("NerveAvalancheUSDPool")
  if (nerveUSDPool) {
    log(`reusing "NerveAvalancheUSDPool" at ${nerveUSDPool.address}`)
    ...
    await save("NerveAvalancheUSDPool", {
      abi: (await get("SwapFlashLoan")).abi,
      address: usdSwapAddress,
    })

    const lpTokenAddress = (await read("NerveAvalancheUSDPool", "swapStorage"))
      .lpToken
    log(`USD pool LP Token at ${lpTokenAddress}`)

    await save("NerveAvalancheUSDPoolLPToken", {
