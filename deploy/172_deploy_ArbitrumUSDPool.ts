...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  let nerveUSDPool = await getOrNull("NerveArbitrumUSDPool")
  if (nerveUSDPool) {
    log(`reusing "NerveArbitrumUSDPool" at ${nerveUSDPool.address}`)
    ...
    await save("NerveArbitrumUSDPool", {
      abi: (await get("SwapFlashLoan")).abi,
      address: usdSwapAddress,
    })

    const lpTokenAddress = (await read("NerveArbitrumUSDPool", "swapStorage"))
      .lpToken
    log(`USD pool LP Token at ${lpTokenAddress}`)

    await save("NerveArbitrumUSDPoolLPToken", {
