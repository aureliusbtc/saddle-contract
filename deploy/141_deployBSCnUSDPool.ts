...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  let nerveUSDPool = await getOrNull("BSCNervenUSDMetaPool")
  if (nerveUSDPool) {
    log(`reusing "BSCNervenUSDMetaPool" at ${nerveUSDPool.address}`)
    ...
    const TOKEN_ADDRESSES = [
      ...
      (await get("NerveBSCUSDPoolLPToken")).address,
    ]
    ...
    log(
      `deployed USD pool clone (targeting "NerveBSCUSDPool") at ${usdSwapAddress}`,
    )
    await save("BSCNerveNUSDMetaPoolDeposit", {
      abi: (await get("MetaSwapDeposit")).abi,
      address: usdSwapDepositAddress,
    })
    log(
      `deployed USD metapool deposit clone (targeting "NerveBSCUSDPool") at ${usdSwapDepositAddress}`,
    )
    await save("BSCNerveNUSDMetaPool", {
    ...
    await save("BSCNerveNUSDMetaPoolLPToken", {
...
export default func
func.tags = ["BSCNerveNUSDMetaPool"]
func.dependencies = [
  "SwapUtils",
  "SwapDeployer",
  "SwapFlashLoan",
